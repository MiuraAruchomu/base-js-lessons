...
function getProductMarkup(product) {
    let randomNumber = Math.round(Math.random() * product.length);
    divProducts.insertAdjacentHTML('afterbegin', `<div class="product"><div>${product[randomNumber].name}</div><img src="${product[randomNumber].imageUrl}" alt=""><div>${product[randomNumber].price}</div><a href="https://example.com/producs/${product[randomNumber].id}">Подробнее</a></div>`);
};
